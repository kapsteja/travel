...
class Admin extends Component  {
    ...
    if (this.state.pageNumber == 2) {
        const file = this.state.addressProof
            const storageRef = ref(storage, `addressProof/${file.name}`)
            const uploadTask = uploadBytesResumable(storageRef, file);
            uploadTask.on("state_changed",
      (snapshot) => {
        const progress =
    ...
    if (this.state.pageNumber == 3) {
        const file = this.state.profilePhoto
      ...
      async()=> {
        ...
        this.closeModal()
        setTimeout(() => {
                const bodyData = {firstName: this.state.firstName, lastName: this.state.lastName, mobile: this.state.mobile, email:this.state.email, drivingPhoto: this.state.drivingLicenseUrl, imgUrl:this.state.addressProofUrl, zone:this.state.zone, altNumber: this.state.altNumber, licenseNumber: this.state.licenseNumber, profilePic: this.state.profilePhotoUrl}
               if(this.state.userType == "vendor") {
                ...
                .then(e => this.setState({pageNumber: 0}))
               }
             },2500)
    }
...
const profilePhotoInput = <div className="form-group m-1">
<label htmlFor="profilePhoto">Profile Photo:</label>
<input
    type="file"
    id="profilePhoto"
    name="profilePhoto"
    onChange={(e) => this.handleUploadChange(e, 'profilePhoto')}
    className="form-control-file"
    required
/>
</div>
                            ...
                            <div className="col-lg-9">
                                ...
                                <div className="row">
                                    <div className=" col-lg-12 text-end  btn-main pull-right">
                                        ...
                                        <button onClick={this.openModal} className="btn btn-primary" style={{ backgroundColor: '#007bff' }}>Add Admin</button>
...
{this.state.pageNumber == 2 && addressProofInput}
{this.state.pageNumber == 3 && profilePhotoInput} 
...
<button type="submit" className="btn btn-primary  m-2" style={{ backgroundColor: '#007bff' }}>Submit</button>
                                ...
                                <div className="card p-4 rounded-5 mb25">
                                    ...
                                    <table className="table de-table">
                                        <thead>
                                            <tr>
                                                <th scope="col"><span className="text-uppercase fs-12 text-gray">vendor name</span></th>
                ...
                <div key={vendor.id}>{vendor.firstname}{vendor.lastname}</div>
            ))}
